...
from ocr import OCRFailure, PaddleOCREngine
...
def run_pipeline(pdf_path: str, doc_id: str, config: PipelineConfig) -> Dict:
	...
	logger = get_json_logger("docai", log_path=output_dir / "pipeline_logs.jsonl")
	...
	try:
		...
		# Detect selectable text to avoid heavy OCR/model downloads when not needed
		try:
	...
	pages = []
	skipped_pages = 0
	# Reserve a small buffer for downstream stages
	...
	for p in iter_pdf_to_images(pdf_path, dpi=config.dpi, max_pages=config.max_pages, deadline_epoch=deadline_epoch):
		pages.append(p)
		# Stop if we're nearing the overall deadline
		if time.perf_counter() >= deadline - 2.0:
			break
	page_images = [p.image for p in pages]
	# If max_pages limited or deadline hit, count remaining as skipped (best-effort)
	...
	if is_digital_pdf:
		...
		try:
			ocr_engine = PaddleOCREngine(
				...
				max_retries=config.max_ocr_retries,
			...
			log_event(logger, "ocr_engine_fallback", reason=str(e), mode=ocr_mode)
	ocr_pages = []
	ocr_failures = 0
	for p in pages:
		if time.perf_counter() >= deadline - 2.0:
		...
		try:
			if ocr_engine is not None:
				ocr_pages.append(ocr_engine.run_page(p.page_index, p.image))
			else:
			...
			ocr_pages.append(type("_", (), {"page_index": p.page_index, "words": [], "texts": lambda self: []})())
	...
	log_event(logger, "stage_ocr", sec=round(ocr_sec, 4), mode=ocr_mode, avg_word_conf=round(avg_ocr_conf, 4), word_counts=word_counts, failures=ocr_failures)
	...
	model = extract_model_name(doc_layout, base_dir=base_dir, region_weight_overrides=getattr(config, "region_weight_overrides", None))
	...
	# Append run log for EDA / error analysis
	run_log = {
		"doc_id": doc_id,
		"language": language,
		"confidence": float(doc_conf),
